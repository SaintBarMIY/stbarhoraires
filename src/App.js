      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          if (fileData.schedules) {
            setAllSchedules(fileData.schedules);
          }

          if (fileData.professorHours) {
            setProfessorHours(fileData.professorHours);
          }
        }
    ...
    reader.onload = async (e) => {
      try {
        ...
        lines.forEach((line) => {
          ...
          // Accepte TAB, virgule ou point-virgule
          const columns = line.split(/\t|,|;/);

          // Il faut 6 colonnes :
          // jour / heure / classe / professeur / cours / local
          if (columns.length >= 6) {
            ...
            const className =
              columns[2]?.trim() || 'Classe inconnue';
            ...
            if (!schedules.professors[profSigle]) {
            ...
            if (!schedules.classes[className]) {
            ...
            // ------------------------------
            // Nombre d'heures par professeur
            // ------------------------------

            profHoursCounter[profSigle] =
              (profHoursCounter[profSigle] || 0) + 1;
          }
        });


        await setDoc(
          doc(db, 'app_data', 'current_schedule'),
          {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      {isModalOpen && selectedEntity && (
        ...
        <ScheduleModal
          ...
          scheduleData={
            allSchedules[activeTab]?.[selectedEntity] || []
          }
          onClose={() => {
